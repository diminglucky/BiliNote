...
> 本项目基于 [JefferyHcool/BiliNote](https://github.com/JefferyHcool/BiliNote) 二次开发，保留原项目 MIT License。当前仓库主要用于本地视频学习笔记生成、截图/视频理解链路调试与定制化改造。
...
原片截图后处理使用 LangGraph 作为正式编排层，负责截图规划、标记过滤、候选帧选择、补充截图和 Markdown 插入。后端依赖 `langgraph`，请通过 `pip install -r backend/requirements.txt` 安装依赖。


截图审稿默认不额外调用多模态 API：`SCREENSHOT_REVIEW_MODE=off` 会使用本地候选帧评分、去重和稳定帧选择。想提升关键图片精准度但控制成本时，推荐 `SCREENSHOT_REVIEW_MODE=balanced`，它只会在段落高价值或候选帧分数接近时调用少量多模态审稿，并受 `SCREENSHOT_VISION_REVIEW_LIMIT` 限制。`assist` 会更频繁调用多模态审稿但允许失败回退；`strict` 强制多模态审稿，质量优先但耗时和 API 成本最高。

常用参数：

```env
SCREENSHOT_REVIEW_MODE=off        # off / balanced / assist / strict
SCREENSHOT_VISION_REVIEW_LIMIT=3  # balanced 模式下每个任务最多审稿次数
SCREENSHOT_CANDIDATE_LIMIT=10     # 每个插图点最多抽取候选帧数量
SCREENSHOT_PLAN_LIMIT=12          # 单个视频最多规划的重点配图点
```
### B 站下载 412 兜底
