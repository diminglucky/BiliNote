...
7. 使用吸引人的标题：''',
        ...
        "tutorial": """9. **教程笔记**: 尽可能详细地记录教程，特别是关键点、操作步骤、代码、命令、配置、报错和运行结果。
如果视频是在讲代码，请保留可复现的代码片段，不要只做概念摘要；音频未完整念出的代码，应结合截图/画面信息补全。"""
    }
...
def get_screenshot_format():
    return '''
11. **原片截图**:
本次最终笔记需要截图，但截图不能反过来破坏文章结构。请先把正文写成高质量学习笔记，再决定是否需要截图标记。

只有以下情况才插入截图标记：
- 画面展示关键代码、配置、命令、文件结构、UI 操作路径、运行结果、错误信息、流程图、对比图。
- 该画面能显著帮助用户理解当前段落，仅用文字描述会损失关键信息。

不要这样做：
- 不要每个章节都插入截图。
- 不要为了凑截图数量插入人脸讲述、过渡页、空白页、重复界面。
- 不要创建“原片截图”集中清单。

如需插入，请放在最相关的段落后，格式必须为：`*Screenshot-[mm:ss]`。
如果不确定具体截图时间，只保留章节标题中的 `*Content-[mm:ss]`，后续视觉流程会根据正文选择截图。
