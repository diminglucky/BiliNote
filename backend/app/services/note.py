...
class NoteGenerator:
    ...
    def _post_process_markdown(
        ...
        if "screenshot" in formats and video_path:
            try:
                updated = self._insert_screenshots(markdown, video_path, audio_meta.duration)
    ...
    def _insert_screenshots(
        ...
        if not matches:
            fallback_timestamps = self._fallback_screenshot_timestamps(duration)
            if not fallback_timestamps:
                return markdown
            lines = ["", "## 原片截图", ""]
            for idx, ts in enumerate(fallback_timestamps):
                try:
    ...
    @staticmethod
    def _fallback_screenshot_timestamps(duration: Optional[float]) -> List[int]:
